...
from tests.test_walking_skeleton_c01_c10 import PydanticAIGatewayAdapter, _free_port, _stop_process
...
async def main():
    ...
    try:
        ...
        async with httpx.AsyncClient(headers=headers) as client:
            r = await client.post(tool_url, json={"jsonrpc": "2.0", "id": "1", "method": "tools/list", "params": {}})
            print("Gateway tools/list initial:", r.json())
        
        # Now down upstream tool
        print("Stopping tool process...")
        _stop_process(tool_proc)
        await asyncio.sleep(0.5)
        
        async with httpx.AsyncClient(headers=headers) as client:
            r2 = await client.post(tool_url, json={"jsonrpc": "2.0", "id": "2", "method": "tools/list", "params": {}})
            print("Gateway tools/list while down:", r2.status_code, r2.text)
            
            r3 = await client.post(tool_url, json={"jsonrpc": "2.0", "id": "3", "method": "tools/call", "params": {"name": "alpha/identify", "arguments": {}}})
            print("Gateway tools/call while down:", r3.status_code, r3.text)
